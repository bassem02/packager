...
/**
 * The class CancelProductTaskExecutor allows canceling a ProductInstance into a
 * separated thread, calling the method cancel on the given product instance.
 * <p>
 * If available (request not null), the Xml properties are given as parameter of
 * the method cancel. Else, null is given.
 * <p>
...
@Component
public class CancelProductTaskExecutor extends AbstractProductTaskExecutor {

	/**
	 * {@link CancelProductTaskExecutor}'s Logger.
	 */
	private static final Log LOGGER = LogFactory.getLog(CancelProductTaskExecutor.class);

	/**
	 * The method run performs the cancellation on the given product.
	 */
	...
	protected void doRun() throws NotFoundException, RestTemplateException {
		try {
			if (!this.product.getCurrentState(productModelProductDriverPortService).equals(State.CANCELED)) {
				try {
					// -- packagerHistory is synchronized because it is used by
					// all products at the same time
					if (this.request == null) {
						synchronized (this.packagerHistory) {
							this.product.cancel(null, packagerHistory, webServiceUserService, productInstanceService,
									productModelProductDriverPortService);
						}
					} else {
						synchronized (this.packagerHistory) {
							this.product.cancel(request.getProperties(), packagerHistory, webServiceUserService,
					...
					if (LOGGER.isWarnEnabled()) {
						LOGGER.warn("Cancel action not supported for product [" + this.product.getIdProductInstance()
								+ "]");
					}
				} catch (Exception ex) {
